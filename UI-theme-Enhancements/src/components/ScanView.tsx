...
function ScanView() {
  ...
  return (
    <div className="flex flex-col h-full bg-white relative overflow-hidden">
      ...
      <div className="absolute bottom-20 right-20 w-96 h-96 bg-green-200 rounded-full mix-blend-multiply filter blur-xl opacity-20 animate-pulse" style={{ animationDelay: '2s' }}></div>
      <div className="absolute top-1/2 left-1/3 w-64 h-64 bg-emerald-300 rounded-full mix-blend-multiply filter blur-xl opacity-15 animate-pulse" style={{ animationDelay: '1s' }}></div>

      <div className="relative z-10 bg-white/80 backdrop-blur-xl border-b border-emerald-200/50 px-6 py-4 shadow-lg">
        <h2 className="text-2xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent">Scanner</h2>
      </div>

      <div className="relative z-10 flex-1 overflow-y-auto p-4 max-h-screen">
        <div className="max-w-2xl mx-auto">
          {!isScanning ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
              <button
                onClick={() => startScan('qr')}
                className="group relative bg-white/70 backdrop-blur-xl rounded-3xl shadow-xl hover:shadow-2xl border border-emerald-200/50 p-10 lg:p-12 transition-all hover:-translate-y-2 hover:bg-white/90 overflow-hidden"
              >
            ...
            <div className="bg-white/80 backdrop-blur-2xl rounded-2xl shadow-xl border border-emerald-200/50 overflow-hidden my-4">
              <div className="bg-gradient-to-r from-emerald-500/90 to-green-500/90 backdrop-blur-xl px-4 py-3 flex items-center justify-between shadow-lg">
              ...
              <div className="p-4 flex flex-col items-center gap-3">
                {scanMode === 'qr' && (
                  <>
                    <div className="relative w-full h-[250px] rounded-xl bg-black flex items-center justify-center overflow-hidden">
                      {capturedImageUrl ? (
                        <img
                    ...
                    <div className="mb-4 p-4 bg-white/70 backdrop-blur-sm rounded-2xl border border-emerald-200/50">
                      <div className="text-center">
                    ...
                    <div className="flex gap-2 bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
                      <button
                        ...
                        className="flex-1 py-2 px-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl hover:from-green-600 hover:to-emerald-600 shadow-sm transition-all flex items-center justify-center gap-1 disabled:opacity-50 text-sm font-medium"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 py-2 px-3 bg-white/70 text-gray-700 rounded-xl hover:bg-white/90 shadow-sm transition-all border border-gray-200 text-sm font-medium"
                      >
                        Reset
                      </button>
                    ...
                    <div className="mt-4 p-4 bg-white/70 backdrop-blur-sm rounded-2xl border border-emerald-200/50 w-full">
                      <p className="text-gray-800 whitespace-pre-line">{status}</p>
                    </div>

                    {/* Enhanced QR Results Display */}
                    {enrichResults && enrichResults.qr_codes && enrichResults.qr_codes.length > 0 && (
                      <div className="mt-6 w-full">
                        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 border border-emerald-200/50 shadow-lg">
                          ...
                          <div className="space-y-3">
                            {enrichResults.qr_codes.map((qr: any, index: number) => (
                              <div key={index} className="bg-white/70 rounded-xl p-4 border border-emerald-200/50 backdrop-blur-sm shadow-sm">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="text-emerald-600 font-bold">QR {index + 1}</span>
                                  <span className="bg-emerald-100/80 text-emerald-700 px-2 py-1 rounded-full text-xs">
                                    {qr.method || 'Unknown method'}
                                  </span>
                                  {qr.parsed && qr.parsed.title && (
                                    <span className="bg-green-100/80 text-green-700 px-2 py-1 rounded-full text-xs">
                                      {qr.parsed.title}
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="mb-2">
                                  ...
                                  <p className="text-sm text-gray-800 bg-gray-50/80 p-2 rounded mt-1 break-all">
                                    {qr.data}
                                  </p>
                                </div>
                                ...
                                {qr.parsed && qr.parsed.details && Object.keys(qr.parsed.details).length > 0 && (
                                  <div className="mt-3">
                                    <span className="text-sm font-medium text-gray-700">Extracted Details:</span>
                                    <div className="mt-1 space-y-1">
                                      {Object.entries(qr.parsed.details).map(([key, value]: [string, any]) => (
                                        <div key={key} className="flex items-start gap-2 text-sm">
                                          <span className="font-medium text-emerald-600 capitalize min-w-20">
                                            {key.replace('_', ' ')}:
                                          </span>
                                ...
                                <div className="mt-3 flex gap-2 flex-wrap">
                                  <button
                                    ...
                                    className="px-3 py-1 bg-emerald-100/80 text-emerald-700 text-xs rounded-lg hover:bg-emerald-200/80 border border-emerald-300/50"
                                  >
                                    Copy Data
                                  </button>
                                  {qr.parsed?.content_type === 'url' && (
                                    <button
                                      ...
                                      className="px-3 py-1 bg-green-100/80 text-green-700 text-xs rounded-lg hover:bg-green-200/80 border border-green-300/50"
                                    >
                                      Open Link
                                    </button>
                                  )}
                      ...
                      <div className="mt-6 w-full max-w-md">
                        <div className="bg-white/80 backdrop-blur-xl rounded-2xl p-4 border border-emerald-200/50 shadow-lg">
                          ...
                          {enrichResults.company_info && enrichResults.company_info.name && (
                            <div className="mb-4">
                              <p className="text-gray-700 text-sm mb-2">Company Information:</p>
                              <div className="p-3 bg-emerald-50/80 rounded-lg border border-emerald-200/50">
                                ...
                                {enrichResults.company_info.website && (
                                  <button
                                    ...
                                    className="text-emerald-600 text-xs hover:text-emerald-700 underline mt-1 flex items-center gap-1"
                                  >
                  ...
                  <>
                    ...
                    <div className="flex gap-2 bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
                      <button
                        ...
                        disabled={isProcessing}
                        className="flex-1 py-2 px-3 bg-gradient-to-r from-emerald-500 to-green-500 text-white rounded-xl hover:from-emerald-600 hover:to-green-600 shadow-sm transition-all flex items-center justify-center gap-1 disabled:opacity-50 text-sm font-medium"
                      >
                        <Camera className="w-4 h-4" />
                        {isProcessing ? 'Processing...' : 'Capture'}
                      </button>
                    ...
                    <div className="mt-4 p-4 bg-white/70 backdrop-blur-sm rounded-2xl border border-emerald-200/50">
                      <p className="text-gray-800 whitespace-pre-line">{status}</p>
                    </div>
                  </>
                )}
