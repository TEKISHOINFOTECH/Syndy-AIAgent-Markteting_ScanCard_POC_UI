...
function DatabaseView({
  ...
  return (
    <div className="flex flex-col sm:flex-row h-full text-gray-100" style={{background: 'linear-gradient(135deg, #0e1a3b 0%, #0f1b3c 60%, #0e1a3b 100%)'}}>
      {loading ? (
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-auto">
          <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 backdrop-blur-2xl rounded-3xl p-3 sm:p-4 border shadow-lg" style={{background: 'rgba(14, 26, 59, 0.9)', borderColor: 'rgba(22, 35, 71, 0.5)'}}>
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 w-full">
                <div className="relative flex-1">
                  <Search className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-gray-400" />
                  <input
                    className="w-full pl-10 sm:pl-12 pr-3 sm:pr-4 py-2.5 sm:py-3 rounded-2xl border text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm sm:text-base" style={{background: 'rgba(22, 35, 71, 0.8)', borderColor: 'rgba(22, 35, 71, 0.5)'}}
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-3 sm:px-4 py-2 rounded-2xl font-medium text-xs sm:text-sm border flex-1 sm:flex-none ${filterType === 'all' ? 'bg-gradient-to-r from-emerald-500 to-blue-500 text-white shadow' : 'text-gray-300 hover:text-white transition-colors'}`.trim()} style={filterType !== 'all' ? {background: 'rgba(22, 35, 71, 0.6)', borderColor: 'rgba(22, 35, 71, 0.5)'} : {}}
                  >
                    All
                  </button>
                  <button
                    onClick={() => setFilterType('active')}
                    className={`px-3 sm:px-4 py-2 rounded-2xl font-medium text-xs sm:text-sm border flex-1 sm:flex-none ${filterType === 'active' ? 'bg-gradient-to-r from-emerald-500 to-blue-500 text-white shadow' : 'text-gray-300 hover:text-white transition-colors'}`.trim()} style={filterType !== 'active' ? {background: 'rgba(22, 35, 71, 0.6)', borderColor: 'rgba(22, 35, 71, 0.5)'} : {}}
                  >
                    Active
                  </button>
                  <button
                    onClick={() => setFilterType('inactive')}
                    className={`px-3 sm:px-4 py-2 rounded-2xl font-medium text-xs sm:text-sm border flex-1 sm:flex-none ${filterType === 'inactive' ? 'bg-gradient-to-r from-emerald-500 to-blue-500 text-white shadow' : 'text-gray-300 hover:text-white transition-colors'}`.trim()} style={filterType !== 'inactive' ? {background: 'rgba(22, 35, 71, 0.6)', borderColor: 'rgba(22, 35, 71, 0.5)'} : {}}
                  >
                    Inactive
                  </button>
                </div>
            ...
            {/* Table Card */}
            <div className="backdrop-blur-2xl rounded-3xl shadow-2xl border overflow-hidden" style={{background: 'rgba(14, 26, 59, 0.9)', borderColor: 'rgba(22, 35, 71, 0.5)'}}>
              {scannedLoading ? (
